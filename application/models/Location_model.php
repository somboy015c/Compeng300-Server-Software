...
class Location_model extends CI_Model
{
    ...
    {
        $this->db->where('countries.name', $name);
        $this->db->limit(8);
        $query = $this->db->get('countries');
        return $query->row();
    }
    ...
    //get state by name
    public function get_state_name($name)
    {
        $this->db->where('states.name', $name);
        $query = $this->db->get('states');
        return $query->row();
    }
    ...
    public function get_states_by_country_name($country_name)
    {
        $country = $this->get_country_name($country_name);
        if (!empty($country)) {
            $this->db->where('states.country_id', $country->id);
            $this->db->order_by('states.name');
            $query = $this->db->get('states');
            return $query->result();
        } else {
    ...
    public function get_cities_by_state_name($state_name)
    {
        $state = $this->get_state_name($state_name);
        if (!empty($state)) {
