...
class Product_controller extends Home_Core_Controller
{
	...
	public function start_selling()
	{
		...
		$data['move'] = $this->input->get('move', true);
		...
		if (user()->role != "member") {
			...
			$data["loc"] = $this->session->userdata('modesy_visitor_default_location');
			...
			if (empty($data["states"]) && empty($data["cities"]) && !empty($data["loc"])) {
				$data["states"] = $this->location_model->get_states_by_country($data["loc"]['country_id']);
				$data["cities"] = $this->location_model->get_cities_by_state($data["loc"]['state_id']);
			}
			...
			if ($data['req'] == 'workshop') {
				...
				$this->load->view('partials/_header', $data);
				if ($data['move'] == 'some') {
					$this->load->view('product/social_media', $data);
				} elseif ($data['move'] == 'coin'){
					$this->load->view('product/contact_info', $data);
				} elseif ($data['move'] == 'pase') {
					$this->load->view('product/payout_settings', $data);
				} elseif ($data['move'] == 'woop') {
					$this->load->view('product/workshop_opening', $data);
				} else {
					$this->load->view('product/profile_update', $data);
				}
				$this->load->view('partials/_footer');
			} else {
