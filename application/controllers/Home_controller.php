	...
	public function index()
	{
		...
		if ($this->default_location_id != 0) {
			if ($this->default_state_id == 0) {
				...
				$key = "latest_products_location_" . $this->default_state_id;
			}
		}
		...
		$data["promoted_works"] = $this->product_model->get_promoted_works();
		$data["products_forsale"] = $this->product_model->get_products_forsale();
		$data["top_products"] = $this->product_model->get_top_products();
		$data["top_works"] = $this->product_model->get_top_works();
		$data["trending_products"] = $this->product_model->get_trending_products();
		$data["trending_works"] = $this->product_model->get_trending_works();
		$data["latest_works"] = $this->product_model->get_latest_works();


		$data["top_shops"] = $this->profile_model->top_members();
		if (!empty($loc)) {
			$data["nearest_shops"] = $this->profile_model->nearest_members($loc['city_id']);
		}
		$data["trending_shops"] = $this->profile_model->trending_members();
		$data["latest_shops"] = $this->profile_model->latest_shops();
		$data["latest_workshop"] = $this->profile_model->latest_workshops();

		$data['just_for_you'] = $this->profile_model->just_for_you();
		$data['most_visited'] = $this->profile_model->most_visited();
...
/**
	...
	public function members()
	{
		...
		// search String translations
		if ($data['search_type'] == "Shop" || $data['type'] == "Shops"){$data["categories"] = $this->category_model->get_parent_categories();} elseif ($data['search_type'] == "Workshop" || $data['type'] == "Workshops") {
			$data["categories"] = $this->category_model->get_subcategories_by_parent_id(12); }

		/*if ($data['search_type'] != "Store" && $data['search_type'] != "Shop" && $data['search_type'] != "Workshop" && $data['search_type'] != "Advert" && $data['search_type'] != "just_for_you" || ($data['search_type'] == "Shop" && $search != 'latest_shops') || ($data['search_type'] == "Workshop" && $search != 'latest_workshops') ||($data['search_type'] == "Advert"  && $search != 'adverts')) {
			redirect(lang_base_url());
		}*/

		if (!empty($data["category"])) { $category_id = $data["category"]->id;
				$data["parent_category"] = $this->category_model->get_category_joined($data["category"]->parent_id);
		} else {  $category_id =  NULL;}

		$loc = $this->session->userdata('modesy_visitor_default_location');
		$link = lang_base_url() . 'members';
		if ($data['search_type'] == "Store" && $search != 'nearest_stores' && $search != 'trending_stores' && $search != 'top_stores' && $search != 'just_for_you' && $search != 'most_visited') { $data['heading'] = 'Stores';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_stores_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_stores($category_id, $pagination['per_page'], $pagination['offset']);
		} elseif ($data['search_type'] == "Store" && $search == 'trending_stores') { $data['heading'] = 'Trending Stores';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_trending_stores_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_trending_stores($category_id, $pagination['per_page'], $pagination['offset']); $search ='';
		} elseif ($data['search_type'] == "Store" && $search == 'nearest_stores') { $data['heading'] = 'Nearest Stores';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_nearest_stores_count($category_id, $loc['city_id']), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_nearest_stores($category_id, $pagination['per_page'], $pagination['offset'], $loc['city_id']); $search ='';
		} elseif ($data['search_type'] == "Store" && $search == 'top_stores') { $data['heading'] = 'Top Stores';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_top_stores_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_top_stores($category_id, $pagination['per_page'], $pagination['offset']);  $search ='';
		} elseif ($data['search_type'] == "Store" && $search == 'most_visited') { $data['heading'] = 'Often Visited Stores';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_most_visited_stores_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_most_visited_stores($category_id, $pagination['per_page'], $pagination['offset']); $search ='';
		} elseif ($data['search_type'] == "Store" && $search == 'just_for_you') { $data['heading'] = 'Stores For You';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_just_for_you_stores_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_just_for_you_stores($category_id, $pagination['per_page'], $pagination['offset']); $search ='';
		} elseif ($data['search_type'] == "Shop" && $search == 'latest_shops') { $data['heading'] = 'Latest Shops';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_latest_shops_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_latest_shops($category_id, $pagination['per_page'], $pagination['offset']); $search ='';
		} elseif ($data['search_type'] == "Workshop" && $search == 'latest_workshops') { $data['heading'] = 'Latest Workhops';
			$pagination = $this->paginate($link, $this->profile_model->get_paginated_filtered_latest_workshops_count($category_id), $this->product_paginate_per_page);
			$data['members'] = $this->profile_model->get_paginated_filtered_latest_workshops($category_id, $pagination['per_page'], $pagination['offset']); $search ='';
		} elseif ($data['search_type'] == "Advert"  && $search == 'adverts') {
			$data["members"] = $this->profile_model->search_ads($category_id); $search ='';
		}
